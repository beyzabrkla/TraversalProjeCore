...
namespace TraversalProjeCore.Areas.Admin.Controllers
{
    ...
    public class VisitorAPIController : Controller
    {
        ...
        public async Task<IActionResult> Index()
        {
            ...
            var responseMessage = await client.GetAsync("http://localhost:5055/api/Visitor");
        ...
        public async Task<IActionResult> AddVisitor(VisitorViewModel visitor)
        {
            ...
            var responseMessage = await client.PostAsync("http://localhost:5055/api/Visitor", content);
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var responseMessage = await client.DeleteAsync($"http://localhost:5055/api/Visitor/{id}");
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var responseMessage = await client.GetAsync($"http://localhost:5055/api/Visitor/{id}");
            if (responseMessage.IsSuccessStatusCode) 
            {
                var jsonData = await responseMessage.Content.ReadAsStringAsync();
        ...
        public async Task<IActionResult> UpdateVisitor(VisitorViewModel visitor)
        {
            ...
            var jsonData = JsonConvert.SerializeObject(visitor);
            StringContent content = new StringContent(jsonData, Encoding.UTF8, "application/json");
            var responseMessage = await client.PutAsync("http://localhost:5055/api/Visitor", content);
